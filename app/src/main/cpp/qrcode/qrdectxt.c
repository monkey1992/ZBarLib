...
static void enc_list_mtf(iconv_tt _enc_list[3], iconv_tt _enc) {
    int i;
    for (i = 0; i < 3; i++)
        if (_enc_list[i] == _enc) {
            int j;
...
int qr_code_data_list_extract_text(const qr_code_data_list *_qrlist,
        ...
        if (!mark[i]) {
            ...
            for (j = 0; j < sa_size && !err; j++, sym = &(*sym)->next) {
                *sym = _zbar_image_scanner_alloc_sym(iscn, ZBAR_QRCODE, 0);
                ...
                if (eci <= QR_ECI_GLI1) {
                    eci = -1;
                    if (eci_cd != (iconv_tt) -1)iconv_close(eci_cd);
                }
            }
            if (eci_cd != (iconv_tt) -1)iconv_close(eci_cd);
            if (!err) {
    ...
    if (utf8_cd != (iconv_tt) -1)iconv_close(utf8_cd);
    if (sjis_cd != (iconv_tt) -1)iconv_close(sjis_cd);
    if (latin1_cd != (iconv_tt) -1)iconv_close(latin1_cd);
    free(mark);
    return ntext;
}
